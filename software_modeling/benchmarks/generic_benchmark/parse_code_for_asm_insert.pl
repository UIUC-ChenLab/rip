sub partition_asm_insert{
my ($current_directory, $benchmark, $source_file) = @_;
my $current_read_line = 0;
my $current_ROI_marker = 0;
my $ROI_start_found = 0;
my $ROI_end_found_flag = 0;
my $target_file_buffer = "";
my $loop_found = 0;
...
while(my $line = <$source_filehandle>){
	...
	if($line =~ m/Start_ROI\(.*\);.*/){
		...
		$line =~ s/Start_ROI\(.*\);/asm("ROILabel$ROI_label_number:");/g;
		$ROI_label_number += 1;
		$ROI_start_found = 1;
	}
	if($line =~ m/Stop_ROI\(.*\);.*/){
		if($ROI_start_found == 1){
			#Replace function with ROI function call
			$line =~ s/Stop_ROI\(.*\);/asm("ROILabel$ROI_label_number:");/g;
			$ROI_label_number += 1;
	...
	if($line =~ m/Start_ROI_loop\(.*\);.*/){
		...
		if (scalar(@function_param_array) == 1){
			$line =~ s/Start_ROI_loop\(.*\);/asm("ROILabel$ROI_label_number:");/g;
			$ROI_label_number += 1;
			$loop_found = 1;
			$ROI_start_found = 1;
		}
	...
	if($line =~ m/Stop_ROI_loop\(.*\);.*/){
		if($ROI_start_found == 1){
			$line =~ s/Stop_ROI_loop\(.*\);/asm("ROILabel$ROI_label_number:");/g;
			$ROI_label_number += 1;
			$ROI_end_found_flag = 1;
			$current_ROI_marker = $current_read_line;
			$ROI_start_found = 0;
...
my $dest_file = $current_directory . "/pin_profile/" . $source_file;
