  ...
  if (isSgIfStmt(node))
    {
      ...
      if (isScop)
	{
	  // Record the if conditional in the control stack.
	  updateControlStack(node);
	  return validNode(node, "Scop OK: ");
	}
				   ...
				   std::set<SgNode*>& scopNodes)
{
  ...
  bool res =
    /// FIXME: LNP,7/1/14: allowing min and max to support i <= min() and i >= max().
    /// But it will incorrectly accept i >= max() and i <= min() too. Result will be
    /// incorrect w/o warning the user in this case for such input programs!
    //parseBoundExpression(conditional, symbols, false, false, true, true, true);
    parseBoundExpression(conditional, symbols, true, true, true, true, true);
  if (res)
    {
...
ScopParser::parseAffineExpression(SgExpression* expr, bool var_ok)
{
  ...
  if (isSgMinusOp(expr))
    expr = isSgMinusOp(expr)->get_operand_i();
