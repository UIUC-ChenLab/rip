			 ...
			 std::map<std::string, SgVariableSymbol*>& map)
{
  ...
  if (i == map.end())
    {
      SgInitializedName* iname = new SgInitializedName(*(v1->get_symbol()->get_declaration()));
      iname->set_name(SgName(fakename));
      SgVariableSymbol* symbol = new SgVariableSymbol(iname);
      // Hack to fool the AST consistency check. Orphan node which
      // should be deleted after the last codegen. However for
      // convenience we do not delete it, to allow re-codegen of a
      // scop.
      symbol->set_parent(v1);
